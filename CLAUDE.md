...
> **RQ2 ✅ answered, both halves.** ⚠️ **Denominator corrected 2026-08-29:** Pyverilog parsed only **262 of 434** analyses across all four sweeps; 172 fell back to Verible (syntax-only — *no structural check ran*, recorded as `parse_ok`). Quote **3 runs with findings in 262 analyses**, never 3 in 434. (The 237/394 figures were the three-sweep subtotal, superseded 2026-09-01.) Worst on the weak benchmark sweep (64/153); the strong sweep reaches 60% (72/120). Detection side unchanged: 93% of 215 injected faults, 0 false positives, 30 of 33 invisible to compiler+simulator.
> **RQ3 ⚠️ partially.** The "informed by Pyverilog" half is answered only in the weakest sense: across 280 runs the static path triggered **exactly one repair** in the 108 runs whose mode permits it; that repair **cleared the finding** and a later compiler-triggered regeneration reintroduced it (see fourth-sweep block below). Comparison half: within either sample no arm beats the control (p=0.125 / p=0.219); **stratified over 64 matched pairs, hybrid beats the control 11–2, p=0.022 — post hoc, uncorrected, report as suggestive.** `retry_only` never beats `baseline` (p=1.000 both samples), so the extra attempt is worth nothing and the gap sits on the diagnosis. Comparison half: hybrid vs `retry_only` gap is **11.4 points** (NOT 17 — old error), p=0.372 unpaired / **p=0.125 McNemar paired**. Arms are circuit-matched so paired tests apply; hybrid vs `pyverilog_only` reaches p=0.012 paired but fails Bonferroni and is uninformative (pyverilog_only never repaired). **Decomposition:** hybrid's 18 passes = 15 first-attempt (baseline 12, same process → noise) + **3 rescued by repair**; the mechanism is worth 6.8 points, not 13.6.
...
### Fourth sweep — `results/verilogeval_strong` (2026-08-29/30, $7.06, 60 runs)

Strong model (`claude-sonnet-4.5`) on the same 20 benchmark circuits; `baseline`, `retry_only`, `hybrid`.
Closes the empty cell in the design (strong model had never been run on the benchmark).

- **Eval1: baseline 5/20 (25%), retry_only 6/20 (30%), hybrid 10/20 (50%).** Weak model was 10% on all.
- ✅ **The control worked.** `retry_only` vs `baseline`: **McNemar p=1.000** (and p=1.000 in the ablation). The bare extra attempt is worth ~nothing. Since hybrid also gets it, the 20-point hybrid–retry gap sits on **the diagnosis**.
- **hybrid vs retry_only: +20 pts, 5–1 discordant, McNemar p=0.219** — not significant at n=20.
- ⚠️ **STRATIFIED (post hoc):** hybrid vs retry_only over ablation (6–1) + strong (5–1) = **11–2 across 64 matched circuits, McNemar p=0.022**. Report as **suggestive, not established**: it is post hoc (sweep run → hybrid seen leading → control added → pooled), fails Bonferroni for 11 comparisons, and the strata differ in model and circuits. See report §7.6 `sec:res-strata`.
- ✅ **Arm isolation verified on data:** all 15 diagnosed repairs are in `hybrid`; zero in `baseline` or `retry_only`.
- **Structural yield unchanged: 1 run with findings in 120 analyses.** Eval1 moved 25 points, structural yield did not. This is the cleanest statement of the central finding.
- **First and only static-triggered repair in the project** (`Prob150_review2015_fsmonehot`, hybrid): iter1 compile → findings appear → iter2 **triggered by static** → *analysis clean, `parse_ok=True`* → iter3 compile → defect returns, ended `exhausted_iters`. ⚠️ **Corrected 2026-09-01:** the repair *worked*; the loop then undid it. Cause: the localiser reads the **generated** DUT (`pyverilog_analysis.py:14`) while evaluation compiles against the **golden** DUT (`evaluate.py:51`), and for this circuit the generated design declares `clk`/`reset` that golden `RefModule` does not. RQ3's "informed by Pyverilog" half is no longer strictly unanswerable.
- **Repair success may depend on the model:** 5/15 (33%) strong vs 3/23 (13%) mixed. Fisher p=0.22 — a direction, not an established difference.
- **Parse coverage improves with the model:** 72/120 = 60% Pyverilog vs 42% on the same circuits with the weak model. Stronger model writes more Verilog-2001, less SystemVerilog.
...
**Totals across all four sweeps: 280 runs, 434 analyses, 262 parsed by Pyverilog, 3 runs with findings, 180/280 runs got ≥1 real structural analysis. Project spend ~$16.50.**

### Added 2026-09-01 — two measurements that bound the null

- ⚠️ **Co-generation confound (found late, measured, NOT excluded before this date).** The localiser reads the **generated** DUT; evaluation uses the **golden** DUT — in all 280 runs. Every check tests agreement between two co-generated artefacts. Re-running the frozen localiser over all 280 stored testbenches against each design (`scripts/reanalyse_against_golden.py`, `results/golden_reanalysis.json`): **generated 2 runs with a finding, golden 7**. Paired on 164 both-parsed runs: 6 golden-only vs 1 generated-only, McNemar **p=0.125**. Co-generation understated the yield ~3× and does **not** account for the null. All 6 extra findings are `Prob150` (generated design invents `clk`/`reset`). All 7 golden-arm findings fall on Eval1-failing runs — zero false positives on real output. Note 2-vs-3: the re-analysis sees only the retained testbench, the sweep analysed every iteration.
- **Parse coverage is a selection effect, not just a gap.** 180 runs Pyverilog could read pass Eval1 at **45.6%**; the 100 that fell back to Verible pass at **6.0%** (Fisher p<0.0001; Eval0 96.1% vs 86.0%, p=0.004). The null is measured on the healthier subset.
- **Selection formula is not recoverable.** The complexity score behind the 20-circuit benchmark subset was never committed; 213 weightings reproduce the quoted 26.4–41.9 / median 18.7. The report now defends the selection by its *outcome* (6–12 ports vs median 4 across the other 136; 15/20 sequential vs 58/136), which is recomputable.
- **Report status:** 48 pages, clean 3-pass build, 0 warnings, 28/28 figures match raw JSON, 22/22 claims trace to source. Verification scripts: `audit_report_figures.py`, `check_report_numbers.py`, `check_tex.py`, `check_style.py`.

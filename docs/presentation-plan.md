...
> **Status (2026-09-24): the deck is built.** `docs/presentation.pptx`, 24 slides, generated by
> `scripts/build_deck.js`. This document is now the *rationale* behind it, kept because the
> argument for each choice is worth having in a viva. Where the two differ, the deck is current.
>
> **What changed from the plan below:** the deck gives **every one of the 13 nodes its own
> slide**, rather than grouping them into three phases as §0 argues. That was a deliberate
> reversal — for a research-project examination the panel wants to see the work, and a node
> the examiner cannot place is worse than a slide they can skim. The backup slides were removed
> and their content moved **inline**, into the speaker notes of the node each question is about:
> the generated-circuit issue is answered at `gen_dut`, the mutant-quality question at
> `evaluate`, the parse-coverage question at `pyverilog_analysis`, and so on. Every slide has
> speaker notes with a SAY section and, where relevant, an IF ASKED section.
>
> **On-slide text is deliberately plain.** The caveats and hedges live in the notes, not on the
> screen — so the slides stay readable and the answer is ready if the question comes.

## Deck as built — 24 slides

| # | Slide | Notes |
|---|---|---|
| 1 | Title | |
| 2 | The problem | the "a testbench that stops looking passes" line |
| 3 | AutoBench and its two gaps | |
| 4 | The pipeline, whole (FIG-1) | roadmap slide |
| 5–17 | **One slide per node**, in order | uniform template: what it does · why we need it · what came out of it, plus a 13-dot progress indicator |
| 18 | Does the checker work? (FIG-3) | hero |
| 19 | What came out of 280 runs (FIG-2) | |
| 20 | Five settings compared (FIG-4) | hero |
| 21 | What triggered every repair (FIG-5) | hero |
| 22 | Why the mistakes disappeared (FIG-6) | |
| 23 | What this contributes | |
| 24 | Thank you | likely questions are in the notes |

**Timing:** 3 setup slides ≈ 2:00 · 14 pipeline slides ≈ 8:00 · 5 results ≈ 6:00 ·
2 closing ≈ 2:00 · ~2:00 spare. The node slides must move at roughly 35 seconds each — they
are uniform on purpose so the audience learns the layout once and then only reads what changed.

**Rebuilding:** `python scripts/make_figures.py --png` then `node scripts/build_deck.js`.
Visual QA: `python scripts/preview_deck.py docs/presentation.pptx`.
